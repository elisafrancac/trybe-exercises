...
const turnArrayIntoFile = async () => {
  const stringsArray = ['Finalmente', 'estou', 'usando', 'Promise.all', '!!!'];
  const createFiles = stringsArray.map((item, index) =>
  ...
  const fileContents = await Promise.all(
    fileNames.map((item) => fs.readFile(item, 'utf-8'))
  );

  const allStringsFile = fileContents.join(' ');
  await fs.writeFile('./fileAll.txt', allStringsFile);
};
turnArrayIntoFile();
